...
class TestMessageBus:
    ...
    async def message_bus(self, mock_redis):
        ...
        with patch('src.core.message_bus.redis.from_url', return_value=mock_redis):
    ...
    async def test_subscribe(self, message_bus):
        ...
        assert test_channel in message_bus._subscriptions
        assert callback in message_bus._subscriptions[test_channel]
    ...
    async def test_unsubscribe(self, message_bus):
        ...
        assert test_channel not in message_bus._subscriptions or \
               len(message_bus._subscriptions[test_channel]) == 0
